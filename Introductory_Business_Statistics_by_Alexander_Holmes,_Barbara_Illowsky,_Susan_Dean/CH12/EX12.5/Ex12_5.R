...
print(paste("variance= ",round(VG_Means,3)))
print(paste("mean = ",round(s_pooled,3)))
print(paste("ms_b= ",round(ms_between,3)))
print(paste("ms_w= ",round(ms_within,3)))
print(paste("f_stat= ",round(f_stat,4)))
alpha=0.03
df1=df_num
df2=df_deno
p_value=round(pf(f_stat,df1=df1,df2=df2,lower.tail=FALSE),4)
print(paste("P(F>0.134) = ",round(p_value,4)))
if(alpha<=p_value)
{
  print("Accept H0")
}else
