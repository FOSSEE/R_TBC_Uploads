...
ggplot(df,aes(x=mid,y=freq))+
 geom_line(aes(x=mid,y=freq),color="pink",size=1,show.legend=FALSE)+
  geom_point(aes(mid,size=0.5),shape=18,show.legend=FALSE)+
    geom_line(data=df2,aes(x=mid,y=freq),color="blue",size=1,show.legend=FALSE)+
  geom_point(aes(mid,size=0.5),shape=18,show.legend=FALSE)+
  scale_x_continuous(breaks=seq(44.5,104.5,10))+
  scale_y_continuous(breaks=seq(0,50,5))+
   xlab("result")+ylab("number of students")